...
#include <QPair>
inline QPointF getCenter(QRectF rect){
    QPointF end = {0,0};
...
inline qreal getRad(QRectF rect){
    return rect.x() + rect.width() - getCenter(rect).x();
}

inline QPair<QPointF, QPointF> GetTangentPointsAtCircle(QPointF center, QPointF dot, qreal R){
